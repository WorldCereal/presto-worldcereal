...
class TestProcessParquet(TestCase):
    def setUp(self):
        ...
        sample_2_data_month = {
            ...
            "timestamp": month_range,
            "start_date": [self.start_date] * self.n_months,
            "valid_date": [self.start_date + pd.DateOffset(months=(MIN_EDGE_BUFFER // 2))]
            * self.n_months,
            "DEM-alt-20m": [np.random.randint(1000, size=1)[0]] * self.n_months,
            "DEM-slo-20m": [np.random.randint(1000, size=1)[0]] * self.n_months,
            "S1-SIGMA0-VV": np.random.randint(1000, size=self.n_months),
        ...
        sample_3_data_dekad = {
            ...
            "valid_date": [
                _dekad_startdate_from_date(
                    pd.to_datetime(self.end_date) - pd.DateOffset(days=10 * (MIN_EDGE_BUFFER // 2))
                )
            ]
            * self.n_dekads,
            "DEM-alt-20m": [np.random.randint(1000, size=1)[0]] * self.n_dekads,
        ...
        sample_4_data_dekad = {
            ...
            "valid_date": [
                _dekad_startdate_from_date(
                    pd.to_datetime(self.end_date) + pd.DateOffset(days=10 * (MIN_EDGE_BUFFER + 1))
                )
            ]
            * self.n_dekads,
            "DEM-alt-20m": [np.random.randint(1000, size=1)[0]] * self.n_dekads,
    ...
    def test_process_parquet_missing_timestamps(self):
        ...
        for freq in self.allowed_freqs:
            ...
            n = 5
            rows_to_remove = df[
                (df["timestamp"] != self.start_date) & (df["timestamp"] != self.end_date)
            ].sample(n)
        ...
        for freq in self.allowed_freqs:
            with self.assertRaises(ValueError):
                process_parquet(
                    ...
                    freq=freq,
                    required_min_timesteps=(self.n_months + 5)
                    if freq == "month"
                    else (self.n_dekads + 5)
                    if freq == "dekad"
                    else None,
                )
